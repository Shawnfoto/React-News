	...
	render() {
		...
		const commnetList = comments.length
			? comments.map((comment, index) => (
				<Card key={index} title={comment.UserName} extra={<a href="#">發布於 {comment.datetime}</a>}>
				...
				<Row>
					<Col span={24}>
						...
						<Form onSubmit ={this.handleSubmit.bind(this)}>
							<FormItem label="您的評論">
								<Input type="textarea" placeholder="寫評論" {...getFieldProps('remark',{initialValue: ''})}/>
							</FormItem>
							<Button type="primary" htmlType="submit">提交評論</Button>
							&nbsp;&nbsp;
							<Button type="primary" htmlType="button" onClick={this.addUserCollection.bind(this)}>收藏該文章</Button>
						</Form>
					</Col>
				</Row>
			</div>
